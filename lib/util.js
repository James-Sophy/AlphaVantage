  ...
  const fn = (type) => (params) =>
    fetch(url(Object.assign({}, params, { function: type })))
      .then(async (res) => {
		...
		const resp = (params.datatype === 'csv') ? await res.text() : res.json();
		if(params.datatype === 'csv'){
			try {
				const respObj = JSON.parse(resp);
				throw respObj;
			} catch(error){
				if(error['Note'] !== undefined) throw `An AlphaVantage error occurred. ${error['Note'] || JSON.stringify(error)}`;
			}
  		}
        return (params.datatype === 'csv') ? csvParse(resp, { columns: true, skip_empty_lines: true }) : resp;
      })
      .then((data) => {
        if (
